...
def jpg2pitches(filePath, data):
    ...
    if __name__ == "__main__" or __name__ == "main":
        ...
        for note_group in note_groups:
            if len(note_group) == 1:
                for note in note_group:
                    if str(note.sym) == "2":
                        # half note
                        dur = beats2duration(8, data.tempo)
                        data.goalPitches.append((str(note.pitch), dur))
                        data.goalNotes.append(GoalNote(dur, note.pitch, data))
                        data.durs.append(dur)
                    else:
                        # quarter note
                        dur = beats2duration(4, data.tempo)
                        data.goalPitches.append((str(note.pitch), dur))
                        data.goalNotes.append(GoalNote(dur, note.pitch, data))
                        data.durs.append(dur)
                    data.currPitch += 1
                ...
                # eighth note
                for note in note_group:
                    dur = beats2duration(2, data.tempo)
                    data.goalPitches.append((str(note.pitch), dur))
                    data.goalNotes.append(GoalNote(dur, note.pitch, data))
                    data.durs.append(dur)
                    data.currPitch += 1
        ...
        contentsToWrite = readFile(filename) + "\n\nSong: " + filePath + " " + str(data.goalPitches)
        ...
        writeFile(filename, contentsToWrite)
